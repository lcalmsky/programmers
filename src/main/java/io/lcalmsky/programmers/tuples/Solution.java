package io.lcalmsky.programmers.tuples;
...
public class Solution {
    ...
    public int[] solution(String s) {
        ...
        for (int i = 0; i < split.length; i++) {
            result[i] = extractElement(split[i], lastSum);
            lastSum += result[i];
        }
        return result;
    }

    private int extractElement(String current, int lastSum) {
        return Stream.of(current.split(",")).mapToInt(Integer::valueOf).sum() - lastSum;
    }
}
